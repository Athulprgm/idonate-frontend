    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          const Icon = st.icon;
          return (
            <div key={st.label} className="bg-white border-slate-200 shadow-sm rounded-3xl p-5 border /80 shadow-xs flex items-center justify-between">
              <div>
      ...
      <AnimatePresence>
        {showAddModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white border-slate-200 shadow-sm rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border relative">
              ...
              <div className="p-6 overflow-y-auto max-h-[80vh]">
                <form onSubmit={handleAddUnitSquadSubmit} className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-3 mt-4 border-t border-slate-100">
                    <button type="button" onClick={() => setShowAddModal(false)} className="flex-1 py-3 bg-white border-slate-200 shadow-sm border text-slate-600 text-xs font-bold rounded-2xl hover:bg-slate-50 transition-all cursor-pointer">
                      Cancel
                    </button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white border-slate-200 shadow-sm rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border relative">
              <div className="bg-gradient-to-r from-amber-500 to-orange-500 p-6 relative flex items-center justify-between text-white">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-white border-slate-200 shadow-sm/20 rounded-xl flex items-center justify-center">
              ...
              <form onSubmit={handleResetPasswordSubmit} className="p-6 space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setResetPasswordModal({ open: false, squad: null, newPassword: '', showPass: false })} className="flex-1 py-3 bg-white border-slate-200 shadow-sm border text-slate-600 text-xs font-bold rounded-2xl hover:bg-slate-50 transition cursor-pointer">
                    Cancel
                  </button>
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl relative">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3 mb-5">
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-900 font-mono bg-white border-slate-200 shadow-sm px-3 py-2 rounded-lg border">{credentialsModal.email}</p>
                </div>
                <div>
